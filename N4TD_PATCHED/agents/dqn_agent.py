...
class DQNAgent(BaseAgent):
    ...
    def _build_network(state_size: int) -> keras.Model:
        """
        Feedforward network với LayerNormalization (thay BatchNormalization).

        Lý do đổi sang LayerNorm:
        - BatchNorm dùng statistics trên mini-batch → không ổn định khi
          batch nhỏ (64) và distribution non-stationary trong online RL.
        - LayerNorm chuẩn hoá theo từng sample → phù hợp hơn với DQN.
        """
        ...
        for units in DQN_HIDDEN_UNITS:
            ...
            model.add(layers.LayerNormalization())   # fix: thay BatchNormalization
