...
def compute_reward(det_ids: list, tl_id: str) -> tuple:
    ...
    for did in det_ids:
        total_wait  += traci.lanearea.getLastStepHaltingNumber(did) \
                       * traci.lanearea.getLastIntervalMeanTimeLoss(did) \
                       if hasattr(traci.lanearea, 'getLastIntervalMeanTimeLoss') \
                       else traci.lanearea.getLastStepHaltingNumber(did)  # fallback
    ...
    info = {
        ...
        'total_wait'  : total_wait,   # bây giờ là tổng waiting time (xe×giây)
    }
    return reward, info
