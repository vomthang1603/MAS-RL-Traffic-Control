...
# ---------------------------------------------------------------------------
# Bước 6: MAS Cooperative Training
# ---------------------------------------------------------------------------

def step6_mas(episodes: int = 50, use_gui: bool = False):
    print("\n" + "=" * 60)
    print("  BƯỚC 6: MAS Cooperative Training (GL1 + GL2 phối hợp)")
    print("=" * 60)
    try:
        import subprocess
        import sys
        result = subprocess.run([
            sys.executable, 'training/train_mas.py',
            '--mode', 'mas',
            '--scenario', 'peak_hour',
        ...
        if result.returncode == 0:
            print("  ✅ MAS training hoàn tất.")
        else:
            print("  ⚠️  MAS training kết thúc với lỗi.")
    except Exception as e:
        print(f"  ⚠️  Bỏ qua bước MAS: {e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--step', type=int, default=1, choices=[1, 2, 3, 4, 5, 6],
    ...
    if 6 in steps:
        step6_mas(episodes=args.episodes_dqn, use_gui=args.gui)
